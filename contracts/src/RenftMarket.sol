...
contract RenftMarket is EIP712 {
  ...
  function borrow(RentoutOrder calldata order, bytes calldata makerSignature) external payable {
    // revert("TODO");
    require(block.timestamp < order.list_endtime, "RenftMarket: order already taken!");
    require(msg.value >= order.min_collateral, "RenftMarket: insufficient collateral!");
    require(order.min_collateral > 0, "RenftMarket: collateral must be bigger than 0");
    require(order.maker != msg.sender, "RenftMarket: maker can't be the same as the taker");

    bytes32 hash = orderHash(order);

    require(orders[hash].taker == address(0), "RenftMarket: order already taken");
    require(!canceledOrders[hash], "RenftMarket: order canceld!");
  ...
  function cancelOrder(RentoutOrder calldata order, bytes calldata makerSignatre) external {
    ...
    address signer = ECDSA.recover(hash, makerSignatre);
    require(signer == order.maker && signer == msg.sender, "RenftMarket: wrong signer");
